...
class TestParseWebUIArgs:
    ...
    def test_no_webui_args(self):
        ...
        assert web_ui_restart is False
        assert port is None
    ...
    def test_webui_restart_enables_webui(self):
        """Test parsing --web-ui-restart enables restart mode."""
        args = ["--web-ui-restart", "--bridge-arg"]
        web_ui, web_ui_only, web_ui_restart, port, config_path, remaining = _parse_webui_args(args)
        assert web_ui is True
        assert web_ui_only is False
        assert web_ui_restart is True
        assert port is None
        assert config_path is None
        assert remaining == ["--bridge-arg"]
...
class TestMainWebUI:
    ...
    def test_main_with_webui_port_and_config_logs_precedence_note(
        ...
        with (
            ...
            patch("mcpbridge_wrapper.webui.config.WebUIConfig") as mock_config_cls,
            patch("mcpbridge_wrapper.webui.server.is_port_available", return_value=True),
...
class TestPortCollisionHandling:
    ...
    def test_occupied_port_in_bridge_mode_skips_webui(
        ...
        with (
            ...
            patch("mcpbridge_wrapper.webui.server.run_server_in_thread") as mock_thread,
            patch("mcpbridge_wrapper.__main__.sys.stderr") as mock_stderr,
        ):
            result = main()
    ...
    def test_occupied_port_with_port_and_config_shows_hint(
        ...
        with (
            ...
            patch("mcpbridge_wrapper.webui.server.is_port_available", return_value=False),
            patch("mcpbridge_wrapper.webui.server.run_server_in_thread") as mock_thread,
            patch("mcpbridge_wrapper.__main__.sys.stderr") as mock_stderr,
        ):
            result = main()
    ...
    def test_occupied_port_in_webui_only_mode_exits_with_error(self, mock_create):
        ...
        with (
            ...
            patch("mcpbridge_wrapper.webui.server.run_server") as mock_run,
            patch("mcpbridge_wrapper.__main__.sys.stderr") as mock_stderr,
        ):
            result = main()
    ...
    def test_toctou_systemexit_from_uvicorn_does_not_crash_thread(self):
        ...
        with (
            patch("mcpbridge_wrapper.webui.server.uvicorn") as mock_uvicorn,
            patch("sys.stderr") as mock_stderr,
        ):
