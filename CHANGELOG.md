...
- Replaced the public legacy MCP lifecycle with MCP `2026-07-28` per-request metadata.
- Adopted the MCP Python SDK v2 and raised the supported Python floor to 3.11.
- Added local `server/discover`, modern `resultType`/`_meta` envelopes, MRTR field preservation,
  and explicit cache hints.
- Kept the per-user broker as a process singleton while isolating request IDs, progress,
  cancellation, and subscriptions by client owner.

### Removed

- Public `initialize` and `notifications/initialized` support. Legacy handshake traffic is
  allowed only inside the private Xcode upstream adapter.
