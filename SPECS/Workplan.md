...
#### ⬜️ P2-T8: Gate broker tools/list on warmed tool catalog
...
- **Acceptance Criteria:**
  - [ ] Broker does not forward client `tools/list` while its internal tool catalog is still cold
  - [ ] Empty or invalid broker `tools/list` probe results do not open the client-facing readiness gate
  - [ ] Cursor/Zed-style first `tools/list` requests receive either a warmed catalog or a clear TTL error, never a prematurely cached empty success
