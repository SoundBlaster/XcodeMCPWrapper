...
#### ⬜️ P1-T14: Document Codex Desktop resource-probe behavior for Xcode tools MCP connectivity
- **Status:** **INPROGRESS**
...
- **Acceptance Criteria:**
  - [ ] Docs explicitly state that `-32601` from `resources/list` / `resources/templates/list` can be expected for this tools-focused server
  - [ ] Docs direct users to validate connectivity via tool calls (`XcodeListWindows`, `XcodeLS`) instead of resource probes
  - [ ] README, docs, and DocC mirrors are aligned and pass repository documentation consistency checks
