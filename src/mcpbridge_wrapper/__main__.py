...
# Private upstream adapter IDs. These messages are never exposed on the public
# modern MCP boundary; they only negotiate with Xcode bridges that still use
# the session-era lifecycle internally.
_DIRECT_UPSTREAM_INIT_ID = -2147483647
_DIRECT_UPSTREAM_READY_TIMEOUT_SECONDS = 10.0
_SUPPORTED_SUBSCRIPTION_FILTERS = (
    "toolsListChanged",
    "promptsListChanged",
...
def main() -> int:
    ...
    pending_methods: Dict[str, str] = {}
    direct_subscriptions: Dict[Any, Dict[str, bool]] = {}
    stdin_closed = threading.Event()
    stdout_lock = threading.Lock()
    direct_upstream_ready = threading.Event()
    direct_upstream_failed = threading.Event()
    upstream_probe_pending = False

    def write_local_response(message: Dict[str, Any]) -> None:
        """Write a response generated by the modern protocol boundary."""
        with stdout_lock:
            sys.stdout.write(encode_message(message) + "\n")
            sys.stdout.flush()

    def acknowledge_subscription(message: Dict[str, Any]) -> bool:
        """Register a modern subscription without forwarding it to Xcode."""
        ...
        honored = {key: True for key in _SUPPORTED_SUBSCRIPTION_FILTERS if filters.get(key) is True}
        direct_subscriptions[subscription_id] = honored
        write_local_response(
            {
                "jsonrpc": "2.0",
    ...
    def route_direct_notification(message: Dict[str, Any]) -> None:
        """Route only subscribed upstream change notifications to the client."""
        ...
        filter_name = filter_by_method.get(method)
        if filter_name is None:
            return

        for subscription_id, filters in direct_subscriptions.items():
            if filters.get(filter_name) is not True:
                continue
            routed = dict(message)
            ...
            meta[SUBSCRIPTION_ID_META] = subscription_id
            params["_meta"] = meta
            routed["params"] = params
            write_local_response(routed)

    # Create request handler callback for stdin forwarder
    def on_request(line: str) -> Optional[bool]:
        """Validate modern requests and handle wrapper-local methods."""
        try:
            message = json.loads(line)
        except (json.JSONDecodeError, ValueError):
            write_local_response(error_response(None, -32700, "Parse error"))
            return False

        if not isinstance(message, dict):
            write_local_response(error_response(None, -32600, "Invalid Request"))
            return False

        protocol_error = validate_request(message)
        if protocol_error is not None:
            write_local_response(protocol_error)
            return False

        method = message.get("method")
        request_id = message.get("id")
        if method == "server/discover" and request_id is not None:
            write_local_response(discovery_response(request_id))
            return False
        if method == "subscriptions/listen" and request_id is not None:
            acknowledge_subscription(message)
            return False
        if method == "notifications/cancelled":
            params = message.get("params")
            target_id = params.get("requestId") if isinstance(params, dict) else None
            if target_id in direct_subscriptions:
                direct_subscriptions.pop(target_id, None)
                return False
        if method in {"initialize", "notifications/initialized"}:
            return False

        if not direct_upstream_ready.wait(timeout=_DIRECT_UPSTREAM_READY_TIMEOUT_SECONDS):
            write_local_response(
                error_response(
                    request_id,
                    -32001,
                    "Xcode upstream handshake did not complete before the request deadline",
                )
            )
            return False
        if direct_upstream_failed.is_set():
            write_local_response(
                error_response(request_id, -32001, "Xcode upstream bridge is unavailable")
            )
            return False
        ...
        try:
            ...
            if metrics is not None:
                ...
                if isinstance(client_info, dict):
                    metrics.set_client_info(
                        str(client_info.get("name", "unknown")),
                        str(client_info.get("version", "unknown")),
                    )
                else:
    ...
    def on_stdin_closed() -> None:
        ...
        if stdin_closed.is_set():
            return
        stdin_closed.set()
        if not direct_upstream_ready.is_set():
            direct_upstream_failed.set()
            direct_upstream_ready.set()
    ...
    # Negotiate privately with Xcode bridges that still require initialize.
    # The public client remains stateless and modern; this message is consumed
    # by the response loop below and never written to stdout.
    try:
        bridge_stdin = getattr(bridge, "stdin", None)
        if bridge_stdin is None:
            # Lightweight test doubles and diagnostic embedders may not expose
            # a writable pipe; retain the old harness behavior for those only.
            direct_upstream_ready.set()
            bridge_stdin = None
        else:
            upstream_probe_pending = True
            upstream_probe = json.dumps(
                {
                    "jsonrpc": "2.0",
                    "id": _DIRECT_UPSTREAM_INIT_ID,
                    "method": "initialize",
                    "params": {
                        "protocolVersion": "2024-11-05",
                        "capabilities": {},
                        "clientInfo": {"name": "mcpbridge-wrapper", "version": "1.0.0"},
                    },
                },
                separators=(",", ":"),
            )
            bridge_stdin.write(upstream_probe + "\n")
            bridge_stdin.flush()
    ...
    try:
        ...
        while True:
            ...
            # Consume the private upstream handshake response. A modern bridge
            # can reject this legacy probe explicitly, in which case its normal
            # request path is already ready and no initialized notification is
            # sent.
            try:
                upstream_message = json.loads(line)
            except (json.JSONDecodeError, ValueError):
                upstream_message = None
            if (
                upstream_probe_pending
                and isinstance(upstream_message, dict)
                and upstream_message.get("id") == _DIRECT_UPSTREAM_INIT_ID
            ):
                upstream_probe_pending = False
                result = upstream_message.get("result")
                ...
                if isinstance(result, dict):
                    try:
                        ...
                        if bridge_stdin is not None:
                            ...
                            bridge_stdin.write(initialized + "\n")
                            bridge_stdin.flush()
                        direct_upstream_ready.set()
                    except (BrokenPipeError, OSError, ValueError):
                        direct_upstream_failed.set()
                        direct_upstream_ready.set()
                elif isinstance(error, dict) and error.get("code") == -32601:
                    # The upstream is already modern and does not know the
                    # private legacy probe; continue without initialized.
                    direct_upstream_ready.set()
                else:
                    direct_upstream_failed.set()
                    direct_upstream_ready.set()
            ...
            if (
                isinstance(upstream_message, dict)
                and "id" not in upstream_message
                and "method" in upstream_message
            ):
                route_direct_notification(upstream_message)
                continue
