...
## Error: "Unknown method 'resources/list'" or "Unknown method 'resources/templates/list'"
...
**Cause:** Xcode MCP integration is tool-oriented. Some client flows probe
resource APIs even when the connected server does not implement resources.

**Solution:** Treat this as non-fatal if tool calls work. Validate with a real
Xcode tool call (for example `XcodeListWindows` or `XcodeLS`) instead of
`resources/*` probes.
...
3. Retry in direct mode first (remove `--broker`), then re-enable broker after
   base connectivity is stable.
